...
export default function CommentList({ videoId }: CommentListProps) {
  ...
  if (loading) {
    return (
      <div className="h-full min-h-0 flex flex-col rounded-lg bg-zinc-900 ring-1 ring-zinc-800 p-5 overflow-hidden">
        <div className="h-6 w-32 rounded bg-zinc-800 animate-pulse mb-4 shrink-0" />
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
          {Array.from({ length: 12 }).map((_, i) => (
            <div
              key={i}
              className="flex gap-3 py-3 border-b border-zinc-800 last:border-b-0 shrink-0"
            >
  ...
  if (error) {
    return (
      <div className="rounded-lg bg-zinc-900 ring-1 ring-zinc-800 p-5 text-center">
          ...
          <>
            ...
            {data.videoComments.pageInfo?.hasNextPage && (
              <button
                ...
                className="w-full mt-4 py-2 text-sm text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors disabled:opacity-50"
              >
